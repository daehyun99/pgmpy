"""Base class for probabilistic classifier."""
...
class BaseProbaClassifier(BaseEstimator):
    ...
    _tags = {
        "object_type": "classifier_proba",  # type of object, e.g., "distribution"
        "estimator_type": "classifier_proba",  # type of estimator, e.g., "regressor"
    ...
    def _check_alpha(self, alpha, name="alpha"):
        ...
        if isinstance(alpha, list):
            if not all(isinstance(a, float) for a in alpha):
                raise ValueError("When `alpha` is passed as a list, it must be a list of floats")
        ...
        for a in alpha:
            if not 0 < a < 1:
                raise ValueError(f"values in {name} must lie in the open interval (0, 1), but found value: {a}.")
    ...
    def _get_columns(self, method="predict", **kwargs):
        ...
        if method == "predict_interval":
            ...
            if coverage is None:
                raise ValueError("coverage must be passed to _get_columns for predict_interval")
            return pd.MultiIndex.from_product([featnames, coverage, ["lower", "upper"]])

        if method == "predict_quantiles":
            alpha = kwargs.get("alpha", None)
            if alpha is None:
                raise ValueError("alpha must be passed to _get_columns for predict_quantiles")
